...
public class Demo3 {
	...
	public static void main(String[] args) throws Exception{
		...
		msg.setFrom(new InternetAddress("\"" + MimeUtility.encodeText("???????") + "\" <itcast_test@sina.com>"));
		msg.setSubject("?????Java???????????????");		
		msg.setReplyTo(new Address[]{new InternetAddress("lili@126.com")});
		msg.setRecipients(RecipientType.TO,InternetAddress.parse(MimeUtility.encodeText("?????") + " <llm@itcast.cn>," + MimeUtility.encodeText("??��??") + " <zxx@itcast.cn>"));
		...
		attch1.setFileName(
				MimeUtility.encodeText("java???.txt")
				);
		...
		htmlPart.setContent("?????Java??????????????????????,???????????????????????????��??????????????????<img src='http://www.itcast.cn/logo.gif'>"
