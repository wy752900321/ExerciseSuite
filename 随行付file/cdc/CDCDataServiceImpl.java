...
import com.tongtech.ti.cdc.common.CDCException;
import com.tongtech.ti.cdc.common.subcription.model.SubFilterConfig;
import com.tongtech.ti.cdc.common.subcription.model.SubscriptionInfo;
import com.tongtech.ti.cdc.dataservice.client.BatchMsgData;
import com.tongtech.ti.cdc.dataservice.client.DataServiceClient;
import com.tongtech.ti.cdc.dataservice.client.RowDataInfo;
import com.tongtech.ti.cdc.dataservice.subscription.Subscriber;
/*
 * CDC服务器的数据服务接口
 */
public class CDCDataServiceImpl {
	...
	public CDCDataServiceImpl(String subName, String cdcServerIP, String cdcServerPort, String cdcServerUser, String cdcServerPassword){
		client = new DataServiceClient(subName,
		         new InetSocketAddress(cdcServerIP, Integer.parseInt(cdcServerPort)), 
		         cdcServerUser,
		         cdcServerPassword, 
		         1000);
		client.connect();
	...
	public CDCDataServiceImpl(String subName, String cdcServerIP, int cdcServerPort, String cdcServerUser, String cdcServerPassword){
		client = new DataServiceClient(subName,
		         new InetSocketAddress(cdcServerIP, cdcServerPort), 
		         cdcServerUser,
		         cdcServerPassword, 
		         1000);
		client.connect();
...
//		client.connect();
		BatchMsgData batchData = client.getBatchMsgData(extractName, batchsize);//获取批量数据
	 ...
	 * 在CDC服务器上添加订阅数据
	 */
	public String startSubscriptionTable(String tableName, String extractName, String schema, LinkedHashMap<String,String> columnInfo) {
		try{
			//连接CDC Client
//			client.connect();
			// 构造订阅信息对象
