...
/**
 * 登陆界面是一具体窗口框 单例设计：一类只设计一个对象来使用的原则
 */
public class LoginFrame extends JFrame {
	...
	private void init() {
		...
		setSize(270, 288);
		setLocationRelativeTo(null);// 先设大小，后居中
	...
	private JPanel createContentPane() {
		...
		// EmptyBorder(int top, int left, int bottom, int right)创建具有指定 insets
		// 的空边框
	...
	private JPanel createBtnPane() {
		...
		p.add(login);
		p.add(cancel);
		// 4实现退出系统功能 2) 绑定到 登录界面的cancel 按钮上
	...
	// 2)在 LoginFrame 界面增加 控制器(ClientContext) 的引用和对象注入代码
	ClientContext clientContext;

	public void setClientContext(ClientContext clientContext) {
		// clientContext的实例通过setClientContext()注入到当前对象的属性this.clientContext中
		// 依赖注入: IOC
		this.clientContext = clientContext;
	}

	/**
	 * 5) 为登录界面增加方法, 用来获取用户的ID和PWD 务必将idField 引用到界面控件对象 务必将pwdField 引用到界面控件对象
	 */
